...
## R-10 pre-dispatch continuation outage qualification fix

A fresh target-host qualification on ADR 0029 source passed R-0 through R-9. R-9 proved the full host-PID-namespace SIGKILL path, Docker restart/PID replacement, exact semantic attempt preservation, generation/fencing +1, checkpoint identity, exactly-one replacement prepare/dispatch/resume chain and `skippedFullAgentInvocation=true`. R-10 then timed out in the qualification-only `r10-continuation-deferred` predicate.

The controller had required `attempts > 0 && last_error`. Runtime increments continuation `attempts` only in `mark_dispatching()`, after target-message lookup and session availability checks and immediately before prompt dispatch. Because R-10 intentionally removes OpenCode before the terminal wake exists, a real outage fails during target-message lookup before `mark_dispatching()`, so the correct state is zero prompt-dispatch attempts with a transport error and a deferred inbox. ADR 0030 replaces the contradictory predicate with a JSON-framed delivery/outbox/inbox proof and keeps accepted/observed or any dispatch during the outage as a Qualification Procedure HOLD. No Runtime continuation semantics changed.
