...
try {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (!['--root','--project-id','--repository-id','--module','--command','--docker-identity'].includes(flag) || (flag !== '--module' && seen.has(flag))) throw new Error('project_cli_argument_invalid');
    seen.add(flag);
  ...
  if (options.command) {
    ...
    if (options.probe) throw new Error('project_cli_command_required');
