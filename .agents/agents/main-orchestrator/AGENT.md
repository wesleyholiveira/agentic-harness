...
4. For a self-contained explicit human delivery turn, call Context Engine MCP `agent_start` with `requestSource="current-user-message"`, omit `request`, and pass the captured `continuation` object in the same call. The trusted OpenCode provenance sidechannel supplies the original human message to Context Engine, so never copy or regenerate a long human request into tool-call JSON. If the current human turn is only a short confirmation/follow-up whose delivery authority lives in accepted repository artifacts, use a compact explicit `request` naming those authoritative paths instead of copying prior conversation text.
