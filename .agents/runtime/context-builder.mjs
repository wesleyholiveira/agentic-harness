...
import { assertExecutableValidationCommands, validationCommandProjectionIssues } from "./validation-command.mjs";
...
export async function buildTaskBrief({ repositoryRoot, registry, plan, task, contextPacket, schemas, maxAttempts = 3, reasoning = null }) {
  ...
  if (taskValidationCommandIds.length > 0) {
    const projectionIssues = validationCommandProjectionIssues({
      commandIds: taskValidationCommandIds,
      commands: taskValidation,
    });
    if (projectionIssues.length > 0) {
      throw new Error(`task_brief_validation_command_projection_invalid:${task.taskId}:${projectionIssues.join("|")}`);
    }
  }
