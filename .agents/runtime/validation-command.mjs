...
const REPOSITORY_RELATIVE_VALIDATION_SCRIPT_EXTENSION = /\.(?:sh|bash|zsh|fish|ps1|cmd|bat|exe|com|py|pyw|js|mjs|cjs|ts|mts|cts|rb|pl|php|jar)$/iu;

// Structured-output schema pattern: require a command-shaped first executable rather than
// accepting arbitrary prose. A slash alone is ambiguous with domain prose such as
// "HTTP/BFF/ML identity tests including N06.", so repository-relative commands must be
// unambiguous: an explicit path prefix, a scripts/bin/tools path, or a script-like suffix.
// Complex validations can always be wrapped in `bash -lc`, `sh -lc`, `pwsh -Command`, etc.
export const VALIDATION_COMMAND_PATTERN_SOURCE = String.raw`^(?:(?:[A-Za-z_][A-Za-z0-9_]*=[^\s]+)\s+)*(?:(?:\.{0,2}[\\/]|~[\\/]|[A-Za-z]:[\\/])[^\s]*|(?:(?:[^\\/\s]+[\\/])*(?:scripts|bin|tools)[\\/][^\s]+|(?:[^\\/\s]+[\\/])+[^\\/\s]+\.(?:sh|bash|zsh|fish|ps1|cmd|bat|exe|com|py|pyw|js|mjs|cjs|ts|mts|cts|rb|pl|php|jar))|(?:bash|biome|bun|cargo|cd|cmake|cmd(?:\.exe)?|corepack|ctest|deno|docker(?:-compose)?|dotnet|env|eslint|export|git|go|gradle|gradlew|java|javac|jest|make|mvn|mvnw|mypy|node|npm|npx|nx|pip|pip3|pipx|pnpm|poetry|powershell(?:\.exe)?|pwsh|py|pytest|python|python3|ruff|rustc|set|sh|tsc|tox|turbo|uv|vitest|yarn|zsh))(?:\s|$)`;
...
export function isExecutableValidationCommand(command) {
  ...
  // A slash alone is ambiguous with domain prose such as HTTP/BFF/ML.
  // Accept repository-relative commands only when the token is structurally
  // unambiguous: conventional scripts/bin/tools placement or script-like suffix.
  ...
  if (/^[^\s/]+(?:\/[^\s/]+)+$/.test(repositoryRelative)) {
    ...
    if (conventionalExecutableDirectory || REPOSITORY_RELATIVE_VALIDATION_SCRIPT_EXTENSION.test(basename)) {
      return true;
    }
  }
  return false;
