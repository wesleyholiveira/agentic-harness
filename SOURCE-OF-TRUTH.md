...
## R-10 OpenCode outage qualification authority

Standalone R-10 treats `agent_continuation_deliveries.attempts` as prompt-dispatch authority. When the qualified OpenCode endpoint is intentionally unavailable before terminal wake delivery, the correct deferral proof is `attempts=0`, `dispatch_started_at=NULL`, no acceptance/observation, a recognized pre-dispatch OpenCode transport error, a published `agent.continuation.wake.v1` outbox identity and the matching Runtime inbox in `deferred` state with the same error. Requiring `attempts>0` would demand an external prompt dispatch while the endpoint is supposed to be unavailable and is therefore not authoritative. After this pre-dispatch proof, OpenCode is restarted and the shared progress-aware continuation observer must prove normal recovery to terminal observation. See ADR 0030.
