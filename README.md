...
- **Portable OpenCode surface:** Main Orchestrator and specialist definitions, project plugins, progress/provenance/continuation tooling and the `sdd` command. Provenance registration follows the same effective Context Engine endpoint as MCP traffic, including consumer/qualification-specific ports.
- **Control-plane-only Main Orchestrator:** project delivery/change work must enter Runtime V2 through `agent_start`; persistent-host edit/write/patch/bash/task/Serena bypasses are denied, while Runtime child specialists retain implementation capability.
- **Host/child process-skill split:** the persistent Main Orchestrator does not load Superpowers bootstrap/design workflows; Runtime-dispatched specialist children retain the pinned Superpowers plugin and skills after ingress.
- **Monotonic Technical Refinement repair:** machine-plan repair re-reviews only the exact incoming `requiredDeltas`; it cannot invent a new same-attempt review scope, and stale blocking/follow-up markers are removed only when explicitly re-reviewed as resolved. Technical Refinement itself has no interactive Superpowers planning workflow.
- **Tool integrations:** Headroom 0.36.5 managed proxy/MCP with its pinned wheel-bundled standalone OpenCode transport entry (`headroom/providers/opencode/_dist/entry.opencode.js`), Serena 1.7.0, Context7, codebase-memory-mcp, optional Caveman and RTK guidance.
...
`harness:opencode` keeps the Agentic Harness as the sole host process supervisor: it resolves the absolute standalone transport entry from the exact `headroom-ai[proxy]==0.36.5` uvx environment, starts the pinned Headroom proxy, waits for readiness, then launches `opencode` directly with that entry in the effective config. Set `AGENT_HARNESS_HEADROOM_ENABLED=false` only as an explicit diagnostic/operator bypass; this removes both the transport plugin and Headroom MCP from the generated host config. Runtime children never inherit the host Headroom integration.

Those are intentionally the only public package scripts. Versioned R12–R17 contract commands are not part of the standalone public API. `harness:migrate` is submodule-safe: it runs the harness-owned migration script inside the consumer-scoped `database-migrate` Compose service, so a clean `.harness` checkout does not need its own `node_modules`. `harness:qualify` is the deterministic standalone promotion controller: it runs outside the operational agent catalog, keeps the Main Orchestrator shell/edit restrictions intact, executes PRE-R0–R-11 directly, and uses the qualified Main Orchestrator only for the normal consumer workload at R-7. R-9 first proves that its qualification-only fault controls are projected into the Runtime worker, then injects an unexpected worker-process SIGKILL at an exact repair checkpoint and requires lease/fencing repair-resume proof rather than treating an administrative Docker restart as process-loss evidence. R-10 proves OpenCode outage before any prompt dispatch (`attempts=0`, no dispatch timestamp, published wake plus deferred inbox) and then proves the same durable continuation reaches terminal observation after host recovery. See `docs/qualification/STANDALONE-PROMOTION.md`.
