...
- **Tool integrations:** Headroom 0.36.5 managed proxy/MCP with the pinned `headroom-opencode@0.36.5` native host transport, Serena 1.7.0, Context7, codebase-memory-mcp, optional Caveman and RTK guidance.
...
`harness:opencode` keeps the Agentic Harness as the sole host process supervisor: it starts the pinned Headroom proxy, waits for readiness, then launches `opencode` directly with the pinned native Headroom transport plugin. Set `AGENT_HARNESS_HEADROOM_ENABLED=false` only as an explicit diagnostic/operator bypass; this removes both the transport plugin and Headroom MCP from the generated host config. Runtime children never inherit the host Headroom integration.
