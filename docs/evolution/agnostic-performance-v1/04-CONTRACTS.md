...
## Amendment R3 — DockerRunnerRef e ToolchainEvidence
Referência normativa: `09-DOCKER-TOOLCHAIN-AUTHORITY.md` e `docker-toolchain-contract.json`. Para projeto containerizado, runnerRef resolve para Docker/Compose, não para executável do host. DockerRunnerRef contém context/daemonRef, composeProject, orderedFiles, profiles, publicConfigDigest, service/replica, purpose, dockerfile/target, image identity, platform, user/cwd, workspace/mount e capability policy. Valores secretos são referências privadas, não valores/digests brutos públicos.
ToolchainEvidence registra phases declared/materialized/toolchain/behavior, actual runner/container ID, imageId/repoDigest quando disponível, source/lock/config identities, executable/version/dependency probes, commandId/argv, timestamps, exit codes e logs sanitizados. Build/test/runtime são alvos diferentes ligados por proveniência. Adicionar runnerEvidenceRef aos handoffs/receipts; identidade da instância é proveniência, não substitui closure hermética.
validationScope é autorização de efeitos; executorLocation é local físico. `workspace` pode ser um workspace isolado DENTRO de Docker. Escolher Docker não permite promover scope para authoritative-host/live, escolher diretório arbitrário ou acessar outro projeto. UNKNOWN/HOLD/FAIL continuam distintos e não autorizam fallback host.
