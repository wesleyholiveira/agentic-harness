...
- `R-5` — effective OpenCode config, exact `headroom-opencode@0.36.5` host transport authority, telemetry-disabled Headroom proxy baseline, and mandatory Context Engine/Serena/Headroom/CBM MCP connectivity.
- `R-6` — fresh OpenCode host launched directly by the qualification controller (never through `headroom wrap opencode`), provenance SHA authority and noReply history probe with zero Runtime run.
- `R-7` — the one intentional model-facing gate: normal consumer workload must enter Runtime via `agent_start`, server-enforced provenance and `runId` before implementation; the accepted current-turn provenance must be durably recorded in PostgreSQL with `historySource=chat-message-hook` rather than inferred from optional logs, and the telemetry-disabled Headroom proxy `/stats.requests.total` counter must increase across that real workload to prove native transport routing.
