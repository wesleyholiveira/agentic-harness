...
The qualification contract suite must prove that a continuation accepted at ten minutes is still eligible when the Rust worker completion budget is fifteen minutes; that explicit ambiguous/manual-review delivery fails immediately; that completion beyond the Runtime deadline is rejected; and that success requires ordered `acceptedAt <= observedAt`. It must also prove that multiline `prompt_text` is observed through JSON framing rather than newline-delimited `psql` rows, that malformed observer shapes fail as Qualification Procedure, and that OpenCode multi-step turns may contain multiple assistant records while the latest child parented by the single deterministic wake is the terminal authority. The live gate must prove exactly one deterministic wake message, one wake-materialized event, one delivered event, and exact delivered-event identity for the latest terminal assistant child. Runtime Rust tests must additionally prove that a latest assistant child with `finish=tool-calls` remains pending even when `time.completed` is present, and that an older terminal-looking child cannot override a newer tool-call child.
