...
The host launcher generates `<AGENT_HARNESS_PROJECT_ROOT>/.runtime/opencode.effective.json` from `config/opencode.template.jsonc`, expands absolute roots, starts the Headroom wrapper by default and exposes the session host on port 4096. The plugin/config source remains harness-owned while generated runtime evidence remains project-owned. Runtime invocation provenance is routed to the same effective Context Engine authority as the generated MCP configuration (including qualification-specific ports), and the Context Engine image packages the same provenance plugin source so SHA validation is against the current revision. Runtime child OpenCode disables nested MCPs that should not be recursively launched inside worker execution.
