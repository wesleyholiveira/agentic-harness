...
Standalone R-8 treats delivery acceptance and assistant completion as separate durable proofs. The controller reads the effective Rust continuation completion budget from the running worker and observes PostgreSQL plus OpenCode history rather than imposing a shorter fixed timeout. Session status is diagnostic only. Terminal delivery states are fail-closed; accepted-but-not-yet-observed delivery remains live until the Runtime completion authority expires. Continuation database observation is JSON-framed rather than line/tab-delimited because `prompt_text` is multiline. Exactly one deterministic user wake remains mandatory, while assistant audit mirrors Rust by selecting the latest assistant child parented by that wake; multi-step tool turns may contain multiple assistant records. The persisted `continuation.delivered` assistant identity must match that latest terminal child. OpenCode tool-call assistant steps are explicitly non-terminal even when they already carry both `finish=tool-calls` and `time.completed`; the Runtime must keep the delivery accepted until the latest child for the deterministic wake reaches a non-tool-followup terminal state.

## Terminal continuation resume ownership

Standalone qualification is host-controller-owned. A terminal Durable Continuation resumes the persistent Main Orchestrator only to observe the delivered run (`agent_summary` exactly once), finish the original user-facing request, and terminate the resumed turn. It must not inherit the older Runtime V2 promotion-controller behavior that told the assistant to finish an `outer-controller procedure`, emit a harness verdict, or start qualification follow-up runs. Same-request `agent_start` re-entry is forbidden after terminal delivery; `context_efficiency` is optional only when the original user request explicitly asks for run-scoped performance/token evidence. See ADR 0025.

R-8/R-10 diagnostics expose bounded assistant tool-call name/status/timestamps without recording tool arguments or output so a real pending tool boundary is visible without changing completion authority.
