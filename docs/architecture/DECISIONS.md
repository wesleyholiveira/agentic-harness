...
- Headroom host transport: the harness resolves the absolute self-contained `headroom/providers/opencode/_dist/entry.opencode.js` from `headroom-ai[proxy]==0.36.5`, supervises that pinned proxy, and launches persistent host OpenCode directly; runtime children do not inherit this host transport.
...
- Product Discovery bootstrap-assessment repair: validate/canonicalize bootstrap-facts/v1 source categories before generic Handoff gating; plain authorized repository path lists may map to repository-context, while anchored locators require bounded schema-constrained projection and only repair failure may consume a full task retry.
