...
function promptFor({ brief }) {
  ...
  const technicalRefinementContract = stage === "technical-refinement"
    ? "- technical-refinement: emit handoff.implementationPlan matching .agents/schemas/implementation-plan.schema.json. Task Brief.upstreamAcceptanceCriteria is immutable Product Owner authority. ownership-projection.json is attached as the compact planning view; the Runtime compiler retains .agents/agents/*/agent.json as exact ownership authority. Explicit owners require a primary/shared/collaborative path match. ownershipMode=fallback-unclaimed-primary may own a path only when no non-fallback implementation agent declares matching primaryPaths; concrete primary domain ownership always wins. Never infer a neighboring package/test path. Acceptance coverage does NOT imply path ownership: when an existing work item materially satisfies another implementation criterion, attach that acceptance ID to the existing item instead of inventing verification/documentation work solely for coverage. Handoff evidence, reconciliation records and verification reports are artifacts, not ownedPaths, unless the product contract explicitly requires that repository file. Work-item validation is scoped; do not copy broad catalog-level defaults. Use executable work-item commands only. Work items may reference only implementation-proof criterion IDs, must be bounded and acyclic, and governance/verification roles cannot own implementation. Technical Refinement approves the executable future-work plan, not an already-executed implementation: never require npm test to have run, implementation files to already exist/change, byte-identical post-state hashes, final diff isolation, QA receipts or Product Acceptance receipts before approval. Encode those future proofs as exact owned paths, acceptance mappings, invariants and validation commands; downstream Implementation/QA/readiness/Product Acceptance own execution evidence. Validate the entire plan against ownership, criteria, dependencies and commands before completion. If valid, emit sddReview with role=technical-lead, stage=technical-refinement, decision=approved, requiredDeltas=[]; otherwise return changes_requested/blocked.\n"
    : "";
...
- quality-assurance: Task Brief.validation is blocking. Task Brief.changeProvenance owns diff attribution; use implementationChangedPaths/implementationReusedPaths, never raw git status alone. Pre-existing dirty/tracked/untracked state is not implementation work. changedPaths=[] with required files in implementationReusedPaths is valid when fresh QA passes. Only increment-caused failures or an explicit repository-wide criterion block. sddReview is a projection of the same QA evidence, not an independent semantic veto: when every assigned criterion is passed, every Runtime-owned blocking validation receipt passed, and there is no blocking: residualRisk or required: followUp, emit decision=approved and requiredDeltas=[]. A negative QA decision must be grounded in one of those evidence channels. If the evidence identifies a repository-fixable code/test/document/config delta, return status=complete with sddReview.decision=changes_requested, a concrete nextRole, and non-empty requiredDeltas so Runtime can re-open bounded implementation repair. Reserve status=blocked and sddReview.decision=blocked for a genuinely unresolved human/external authority or environmental blocker that repository implementation cannot repair.
      ...
      try {
        ...
        if (handoff.sddReview?.decision === "approved") {
          ...
          emitRepairEvent("repair.completed", {
            ...
            repairMutationScope: repair.repairMutationScope, repairEvidence: repair.repairEvidence ?? [],
          });
          emitRepairEvent("retry.full_attempt_avoided", {
