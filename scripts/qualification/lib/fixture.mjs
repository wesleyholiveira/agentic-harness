...
export const FIXTURE_FILES = Object.freeze({
  ...
  "docs/specs/qualification/r9/PRD.md": `# R-9 PRD — Isolated initials helper

## Context

The qualification consumer already has an established \`formatName(name)\` implementation and tests from the earlier semantic workload. R-9 qualifies physical worker-loss recovery, so its implementation slice must be additive and isolated from that established behavior.

## Goal

Add an exported \`formatInitials(name)\` helper in the new file \`src/format-initials.mjs\` and add its tests in \`test/format-initials.test.mjs\`.
...
FR-3. For a non-empty value, split on one-or-more whitespace characters and concatenate the uppercase first character of every non-empty word.

FR-4. Do not modify \`src/format-name.mjs\` or \`test/format-name.test.mjs\`; their established behavior is an R-9 baseline invariant.

FR-5. Do not add runtime or test dependencies.

## Acceptance criteria

AC-R9-1. \`formatInitials("Wesley") === "W"\`.

AC-R9-2. \`formatInitials("Wesley Oliveira") === "WO"\`.

AC-R9-3. \`formatInitials("  wesley   oliveira  ") === "WO"\`.

AC-R9-4. \`formatInitials("") === "Anonymous"\`.
...
- no modification of \`src/format-name.mjs\` or \`test/format-name.test.mjs\`;
